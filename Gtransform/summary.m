...
for N = Ns
    ...
    se_res = zeros(m, p);
    infer = true;
    res_file = strcat('../res/Gtransform/res_Gtransform_summary_N', num2str(N),...
    ...
    if isfile(res_file)
        ...
        for seed = 1:m
            ...
            load(strcat('../res/Gtransform/res_Gtransform_N', num2str(N),...
                '_seed', num2str(seed), ...
...
%     IMSE_h0 = res(:,ii+2*tol+1);
    ...
    temp= sort(runtime);
    runtime = temp(6:(m-5));
    res_ode(idx,:) = [mean(IMSE_H0), std(IMSE_H0), mean(runtime), std(runtime)];
    idx = idx+1;
end

disp(res_ode)
res_npmle = zeros(length(Ns), 4);
idx = 1;
for N = Ns
    load(strcat('../res/baseline/NPMLE_output_N', num2str(N),'_m', num2str(m), '.mat'),...
        'output','runtime','imse');
    if N == 1000
        runtime_base = mean(runtime);
    end
    ...
    temp= sort(runtime);
    runtime = temp(6:(m-5));
    res_npmle(idx,:) = [mean(imse), std(imse), mean(runtime), std(runtime)];
    idx = idx+1;
end
disp(res_npmle)
...
fontsize = 12;
subplot(1,2,1);
errorbar(Ns, res_ode(:,1), res_ode(:,2),'LineWidth',4);
hold on 
errorbar(Ns, res_npmle(:,1), res_npmle(:,2), 'r', 'LineWidth',2);
hold off
xlim([900 8500])
xticks(Ns)
xticklabels({'1', '2', '4', '8'})
...
legend({'ODE-LT', 'NPMLE'}, 'Location', 'northeast')
set(gca,'FontSize',fontsize)

subplot(1,2,2);
errorbar(Ns, res_ode(:,3), res_ode(:,4),'LineWidth',2);
hold on 
errorbar(Ns, res_npmle(:,3), res_npmle(:,4), 'r', 'LineWidth',2);
hold on
x = 1000:8000;
y = x/1000*res_ode(1,3);
plot(x,y,'LineStyle', '--', 'Color', [0.4660, 0.6740, 0.1880], 'LineWidth',2);
hold on
x = 1000:8000;
y = x.^2/1000^2*res_npmle(1,3);
...
legend({'ODE-LT', 'NPMLE', 'Reference O(N)', 'Reference O(N^2)'}, 'Location', 'northwest')
set(gca,'FontSize',fontsize)
set(gca,'yscale','log')
set(gca,'xscale','log')
set(gcf, 'Position', [0, 0, 600, 250]);

print(gcf,strcat('../res/fig/lt_ode_summary', '.png'),'-dpng','-r500')
